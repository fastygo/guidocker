...
func (h *DashboardHandler) executeView(w http.ResponseWriter, name string, data interface{}) {
	...
	switch name {
	case "overview":
		view, ok := data.(views.OverviewView)
		if !ok {
			err = errors.New("invalid overview view model")
			break
		}
		component = pages.OverviewPage(view)
	case "apps":
		view, ok := data.(views.AppsView)
		if !ok {
			err = errors.New("invalid apps view model")
			break
		}
		component = pages.AppsPage(view)
	case "app_detail":
		view, ok := data.(views.AppDetailView)
		if !ok {
			err = errors.New("invalid app detail view model")
			break
		}
		component = pages.AppDetailPage(view)
	case "app_detail_paas":
		view, ok := data.(views.AppDetailPaasView)
		if !ok {
			err = errors.New("invalid app detail paas view model")
			break
		}
		component = pages.AppDetailPaasPage(view)
	case "compose":
		view, ok := data.(views.ComposeView)
		if !ok {
			err = errors.New("invalid compose view model")
			break
		}
		component = pages.ComposePage(view)
	case "compose_container":
		view, ok := data.(views.ComposeContainerView)
		if !ok {
			err = errors.New("invalid compose container view model")
			break
		}
		component = pages.ComposeContainerPage(view)
	case "logs":
		view, ok := data.(views.LogsView)
		if !ok {
			err = errors.New("invalid logs view model")
			break
		}
		component = pages.LogsPage(view)
	case "logs_container":
		view, ok := data.(views.LogsView)
		if !ok {
			err = errors.New("invalid logs container view model")
			break
		}
		component = pages.LogsContainerPage(view)
	case "settings":
		view, ok := data.(views.LayoutData)
		if !ok {
			err = errors.New("invalid settings view model")
			break
		}
		component = pages.SettingsPage(view)
	case "scan":
		view, ok := data.(views.ScanView)
		if !ok {
			err = errors.New("invalid scan view model")
			break
		}
		component = pages.ScanPage(view)
	default:
		err = errors.New("unknown template: " + name)
	}
	...
	if renderErr := component.Render(context.Background(), w); renderErr != nil {
		log.Printf("Render %s: %v", name, renderErr)
		http.Error(w, "Internal Server Error", http.StatusInternalServerError)
	}
}
